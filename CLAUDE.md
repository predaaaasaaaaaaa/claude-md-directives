...
When given a bug report: just fix it. Don't ask for hand-holding. Trace logs, errors, failing tests - then resolve them. Zero context switching required from the user. Go fix failing CI tests without being told how.
...
When the same edit needs to happen across many files, suggest parallel batches via /batch. Verify each change in context.
